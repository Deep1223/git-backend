    ...
    try {
        ...
        if (resolvedSortField === 'module') {
            ...
            const pipeline = [
                ...
                {
                    $addFields: {
                        __moduleSortKey: { $toLower: { $ifNull: ['$module', ''] } },
            ...
            modules = await ModuleMaster.aggregate(pipeline).collation(collation);
        } else {
            // Regular find with projection
            let query = ModuleMaster.find(filter)
                .collation(collation)
                .sort(sort)
                .skip(skip)
                .limit(limit);
        ...
        const totalCount = await ModuleMaster.countDocuments(filter);
...
exports.getModuleById = async (req, res) => {
    try {
        const module = await ModuleMaster.findById(req.params.id);
        if (!module) {
...
exports.createModule = async (req, res) => {
    try {
        ...
        const module = await ModuleMaster.create(req.body);
        res.status(201).json({
            success: true,
            data: module
        });
...
exports.updateModule = async (req, res) => {
    try {
        ...
        let module = await ModuleMaster.findById(id);
        ...
        module = await ModuleMaster.findByIdAndUpdate(id, req.body, {
            new: true,
            runValidators: true
        });
    ...
    try {
        ...
        if (typeof idData === 'object' && !Array.isArray(idData)) {
            ...
            if (selectall) {
                ...
                await ModuleMaster.deleteMany(query);
            } else {
